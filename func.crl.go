...
func CreateCRLObject(certList []pkix.RevokedCertificate, key crypto.Signer, issuingCert *x509.Certificate, expiryTime time.Time) ([]byte, error) {
	if certList == nil {
		return nil, &errorString{"Missing certificate list required to create CRL."}
	}
	if key == nil {
		return nil, &errorString{"Missing issuing key required to create CRL."}
	}
	if issuingCert == nil {
		return nil, &errorString{"Missing issuing certificate required to create CRL."}
	}
